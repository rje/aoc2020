// giving up on day 19, implementation from
// https://github.com/dhconnelly/advent-of-code-2020/blob/main/day19/src/main.rs
...
impl Matcher {
    ...
    fn matches_all(&self, s: &str, mut q: &mut Vec<usize>) -> bool {
        ...
        match first {
            ...
            Rule::Alt(seq1, seq2) => {
                self.match_seq(&seq1, s, &mut q.clone()) || self.match_seq(&seq2, s, &mut q.clone())
            }
        }
    }
...
fn main() {
    let path = "data/day19/problem_1.txt";
